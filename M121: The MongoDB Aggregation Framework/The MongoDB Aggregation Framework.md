...
So let's get started working with facets and explore this new functionality that [INAUDIBLE] brings. Now, to get a better picture of what we are going to do, let's imagine the following scenario. Let's imagine that we have an application that has some sort of search bar where we can look for things like MongoDB, for example. Once we press the Enter button looking for this particular keyword or term, we are generally prompt with some list of results.

We might have some attributes, some indication of things related with this term given the catalog that we are searching on. But we also might have some sort of filters or characterization for this search term in combination with several different dimensions that this information that we are storing in the catalog of this application might have. So to explain this very well, we're going to be using a very dear data set to you guys that we've been exploring throughout the course.

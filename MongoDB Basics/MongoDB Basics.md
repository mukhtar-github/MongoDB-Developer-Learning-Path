...
We are going to skip the *Explain Plan* tab for now and look at the *Index* tab. Currently, we only have one *Index*, which is the default *Index in MongoDB*. All documents are indexed by their *_id* value. As we learned earlier in the course, to increase query performance, we need more than just an *_id index*. Unlesss, of course, we only query data by it's *_id* value,
